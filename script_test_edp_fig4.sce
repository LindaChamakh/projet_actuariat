...
N_x = 20;//nb de points pour k_x
N_sigma = 20//nb de points pour sigma_x
...
for i=1:N_x
    ...
    for j=1:N_sigma
        ...
        resultat(j,i) = (phi((ind_r+1)*(M_1-1) + ind_x) - phi(ind_r*(M_1-1) + ind_x))/(r(ind_r+1)-r(ind_r));
    end
end
printf('\ntemps nécessaire  : %.3f\n',toc());
...
a = gca();
a.rotation_angles=[60,260];
xtitle('$\partial_{r} \psi\ à\ t=0\ en\ (x_0,r_0)$')
xlabel("$\sigma_x$","fontsize",6);
ylabel("$\kappa_x$","fontsize",6);
zlabel("$\partial_{r} \psi\$","fontsize",6);
